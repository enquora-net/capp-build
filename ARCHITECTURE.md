...
* Cyclic Dependencies: Circular imports (e.g., File A imports File B imports File A) must be detected and reported as actionable errors rather than allowing the compiler to enter an infinite loop.
...
This is more than a simple enumeration; it is a formal declaration of the toolchain's architecture. Execution is driven directly by the enum via an `impl` block. Adding a variant to `BuildPhase` without a corresponding arm in `run()` is a compile error — the binding between declaration and execution is enforced by the type system, not by convention:
...
impl BuildPhase {
  ...
  fn run(self, ctx: Ref<BuildContext>) -> Result<(), error> {
    match self {
      ...
      // ... all eight variants must be handled or the build fails
    }
  }
}
```

The pipeline then reduces to a single expression:
...
`all()` declares execution order. `run()` enforces completeness. The two concerns are separated but co-located in the same `impl` block, immediately adjacent and visible together.

> **Note:** Lisette currently restricts integer-backed value enums to typedef files generated by its Go interop tooling, preventing direct range iteration over `BuildPhase`. The `all()` function is the idiomatic workaround within the current language. When Lisette lifts this restriction, `all()` will be replaced by a range expression over an integer-backed enum, eliminating the one manually-maintained list that the compiler cannot currently verify.
...
The compiler exposes two orthogonal dimensions of output control. They are independent of each other and both are always meaningful regardless of the build mode in use.

**Build mode** governs what transformations are applied to the source:

```rust
pub enum BuildMode {
  Debug,    // source maps, no optimisation
  Release,  // optimisation, no source maps
  Clean,    // remove build artefacts without compiling
}
```

**Delivery context** governs how the compiled output is structured for the target server environment. It is a property of the deployment target, not of the source code:

```rust
pub enum DeliveryContext {
  HTTP1,  // concatenated, minified — works on any server (default)
          // minimises round trips on HTTP/1.x connections
  HTTP2,  // per-file output — opt-in for correctly configured servers
          // exploits multiplexing and fine-grained cache granularity
}
```

HTTP/1.x is the default because most Cappuccino applications are served from general-purpose servers where HTTP/2 has not been explicitly configured. Developers who have correctly configured HTTP/2 opt in explicitly with `--http2`.

At the command line:

```
capp-build build [path] --mode debug|release|clean [--http2]
```

Additional output dimensions — pre-compression, dead code elimination, and similar post-processing — are expressed as further independent flags following the same pattern. Each flag governs exactly one concern; no flag interacts with another.
...
- **CLI Surface**: Managed via [cobra](https://github.com/spf13/cobra), providing the `capp-build build`, `verify`, and `smoke` commands.
...
- **Straightforward Concurrency**: Efficient parallelization of build tasks.
